...
diag_log format ["UNIT INIT : %1", [_this, side _unit, _type]];
    ...
    case (side _unit != west && side _unit != civilian) : {
        ...
        if (!isDedicated) then {
            ...
            if !(_type in btc_actions_units) then {
                ...
                private _action = ["Search_intel", "Search for intel", "\A3\ui_f\data\igui\cfg\simpleTasks\types\search_ca.pa", {(_this select 0) spawn btc_fnc_info_search_for_intel;}, {!Alive (_this select 0)}] call ace_interact_menu_fnc_createAction;
    ...
    case (side _unit isEqualTo civilian) : {
        ...
        if (!isDedicated) then {
            ...
            if !(_type in btc_actions_units) then {
                ...
                sleep 1;
                private _action = ["Civil_Orders", "Civil Orders", "\A3\ui_f\data\igui\cfg\simpleTasks\types\meet_ca.paa", {}, {true}] call ace_interact_menu_fnc_createAction;
                [_type, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;
