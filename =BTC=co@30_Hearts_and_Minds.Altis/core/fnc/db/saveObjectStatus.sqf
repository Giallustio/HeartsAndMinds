...
if !(!isNil {_object getVariable "loaded"} || !Alive _object || isNull _object) then {
		...
		private _cargo = [];
		{_cargo pushBack [(typeOf _object),(_object getVariable ["ace_rearm_magazineClass",""]),[getWeaponCargo _object,getMagazineCargo _object,getItemCargo _object]]} foreach (_object getVariable ["cargo",[]]);
