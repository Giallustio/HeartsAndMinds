params [
    ["_type", 0], // 0 - Rifleman, 1 - Medic Adv, 2 - Medic Basic, 3 - Repair, 4 - Engineer, 5 - Anti-Tank, 6 - Anti Air, 7 - Sniper, 8 - Machine gunner
...
(_arsenal_loadout apply {_x select _color}) params ["_uniform", "_vest", "_helmet", "_hood", "_laserdesignator", "_night_vision", "_weapon", "_weapon_sniper", "_weapon_machineGunner", "_pistol", "_launcher_AT", "_launcher_AA", "_backpack", "_backpack_big", "_radio"];
...
private _cfgPatches = configFile >> "CfgPatches" >> "task_force_radio";
if (_isDay) then {
    [
        [_weapon, "", "", _optics select _isDay, [_weaponMagazine, _weaponCount], [], ""],
        _launcher_array,
        ...
        ["ItemMap", "B_UavTerminal", ["ItemRadio", _radio] select (isClass _cfgPatches), "ItemCompass", "ItemWatch", ""]
    ]
} else {
    [
        [_weapon, "muzzle_snds_65_TI_blk_F", "acc_pointer_IR", _optics select _isDay, [_weaponMagazines param [1, _weaponMagazine], _weaponCount], [], ""],
        _launcher_array,
        [_pistol, "", "", "", [_pistolMagazine, _pistolCount], [], ""],
        [_uniform, _cargo_uniform],
        ...
        _cargos select _type, _helmet, _hood, _binocular_array,
        ["ItemMap", "B_UavTerminal", ["ItemRadio", _radio] select (isClass _cfgPatches), "ItemCompass", "ItemWatch", _night_vision]
    ]
};
