...
_action = ["Interrogate_intel", "Interrogate", "", {[(_this select 0),true] spawn btc_fnc_info_ask;}, {(Alive (_this select 0) && {[_unit] call ace_common_fnc_isAwake} && captive (_this select 0))}] call ace_interact_menu_fnc_createAction;
{[_x, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;} foreach btc_type_units;
